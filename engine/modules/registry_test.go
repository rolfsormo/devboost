...
// TestAllResourcesResolveOnLinuxWithVendorInstalls is the real
// end-to-end proof of the NeedsProvider mechanism: on Ubuntu/Fedora,
// mise/atuin are NOT provided by base_packages (they're real apt/dnf
// gaps — see linuxvendor.go) but BY linuxvendor.go's own
// vendor_install_* resources instead. Mise()/Services() (Darwin-only
// for the latter) must still resolve correctly against whichever
// concrete resource actually Provides what they need on THIS platform
// — this is the scenario a real Ubuntu container run originally
// surfaced as broken (mise_toolchains silently never running).
func TestAllResourcesResolveOnLinuxWithVendorInstalls(t *testing.T) {
	for _, os := range []kinds.OS{kinds.OSLinuxUbuntu, kinds.OSLinuxFedora} {
		t.Run(string(os), func(t *testing.T) {
			home := t.TempDir()
			t.Setenv("HOME", home)
			cfg := loadFixtureConfig(t, "")

			resources := AllResources(cfg, os)
			if len(resources) == 0 {
				t.Fatal("expected at least some resources")
			}
			if _, err := engine.ComputeDiff(resources); err != nil {
				t.Fatalf("registry produced resources that don't resolve on %s: %v", os, err)
			}

			foundMiseProvider := false
			for _, r := range resources {
				if containsString(r.Provides, "mise") {
					foundMiseProvider = true
				}
			}
			if !foundMiseProvider {
				t.Fatalf("expected some resource to Provide \"mise\" on %s (either base_packages or vendor_install_mise)", os)
			}
		})
	}
}
